...
function UserDetailsHooks(UserName) {
  ...
  const getData = async () => {
    ...
    if (UserName) {
      try {
        ...
        const resp = await fetch(`https://api.github.com/search/users?q=${UserName}`);
  ...
  useEffect(() => {
    getData(); // Call the getData function when the component mounts or when the query state changes.
